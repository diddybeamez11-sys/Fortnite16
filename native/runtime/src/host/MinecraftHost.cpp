...
constexpr const char* TARGET_BUILD_ID = "665d33595ddec90fe7347a45cd4da65e6f0871aa"; // 1.21.80.3 ARM64
...
void MinecraftHost::bootstrapLoop() {
    ...
    while (running_.load()) {
        ...
        if (!verifyTarget()) {
            ...
            updateStatus("Minecraft runtime found; waiting for verified 1.21.80.3 ARM64 profile");
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            continue;
        }

        if (installRenderHook()) {
            __android_log_print(ANDROID_LOG_INFO, TAG, "E-Client in-game host ready");
            return;
        }
